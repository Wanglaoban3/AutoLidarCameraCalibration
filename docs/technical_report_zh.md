...
## 11. Sweep-TEED 联合精修实验与当前最佳方案

在 `scene-0061` 上，以第 6 节的 ICP+手眼结果为粗标，注入同一组 3/−3/4 度噪声。历史 LiDAR 点通过 nuScenes `ego_pose` 补偿到参考时刻，动态框内点被移除。图像侧使用官方 TEED BIPED epoch-5 模型；场景的 234 张关键帧相机图已预计算并缓存，因此参数实验不重复图像推理。

### 11.1 推荐的当前流程

1. 原始 LiDAR ICP + 手眼标定产生共同车体系粗修正。
2. 以 20 个连续 LiDAR sweep 为一个滑动窗口，而不是将所有历史扫帧堆入单个窗口。
3. 在地面平面内取强度 P70 候选，栅格化为 0.1 m BEV 图，并仅保留高反射区域边界。
4. 将 BEV 边界投影到六相机缓存的 TEED 距离场；每次优化重新查询距离场，不固定初始匹配。
5. 地面法向提供 roll/pitch 局部先验，地面标识主要用于 pitch/yaw 的残差下降与留出窗口验证。
6. 平移默认冻结。平面车辆运动和道路标识不足以稳定观测三维平移。

该流程是当前仓库中精度最高、且工程上最可解释的候选方案；但当前数据规模尚不满足发布门槛。

### 11.2 结果对比

| 方法 | roll/pitch/yaw 真值误差 (deg) | 留出集变化 | 结论 |
|---|---|---|---|
| ICP+手眼粗标 | `[0.791, -0.766, 0.194]` | 基线 | 可作为精修初值 |
| 20-sweep P70 地面 BEV+TEED，局部约束 | `[-0.313, -0.346, -0.223]` | 中位 `12.31 -> 11.28 px`；P90 `39.94 -> 41.24 px` | **真值误差最低**，但触边界且留出 P90 变差，不发布 |
| 六相机 20-sweep 地面 BEV+TEED | `[-0.613, -0.015, -0.158]` | 中位 `13.18 -> 12.73 px`；P90 `38.11 -> 36.47 px` | 图像残差更稳，但 roll 触边界，不发布 |
| 20-sweep 竖直深度边 | `[-0.568, -0.015, -0.158]` | 留出中位略变差 | 竖直点稀疏，拒绝 |
| 40-sweep 竖直深度边 | `[0.587, -0.015, -0.158]` | 残差下降但 roll 反向 | 长窗口增加错误对应，拒绝 |
| 时间追踪 + TEED 一致竖直实例 | `[-1.613, -0.015, -0.158]` | 留出 P90 未改善 | 16 个实例仍非同一物理边，拒绝 |

因此，本实验不能证明稳定达到小于 `0.3 deg`。最接近的单次真值结果仍有 `0.346 deg` 的 pitch 误差，且没有通过独立验证。

### 11.3 已验证的经验

- **叠帧有效，但窗口不能无限加长。** 40-sweep 原始深度边候选为 6,307 个，竖直边由 20-sweep 的 7 个增至 44 个；同时树冠、路缘和遮挡层也急剧增加。20-sweep 是当前更合理的局部密度与拖影折中。
- **地面与竖直物体必须分支处理。** 地面强度轮廓对 pitch/yaw 有信息；竖直轮廓理论上帮助 roll，但不能混成同一残差池。
- **TEED 比固定 Canny/LSD 匹配更适合迭代。** 距离场可在每个候选外参下重新查询，不会锁死第一次的错误线段对应。缓存 234 张图后，后续实验的 `inferred` 应为零。
- **时间稳定不等于几何正确。** 3D 追踪得到 149 条稳定轨迹，TEED 一致性筛到 16 条，但它们仍会落在树冠、车辆邻近边和立面纹理上，导致 roll 向错误方向漂移。
- **残差下降不是标定成功。** 优化触边界、留出 P90 不改善、或真值方向反转时必须拒绝发布，即使训练残差和留出中位数略有下降。

### 11.4 明确否定的方案

不应继续使用“40-sweep 全部深度边 + 通用 TEED 竖边”或“时间追踪 + 线段近邻”直接更新 roll。下一版若要恢复 roll，必须引入真正的实例级语义：杆/树干/立柱掩码或实例线跟踪，并验证同一 3D 实例在多个图像窗口中对应同一个语义对象。该工作应作为独立研究项，而不是放宽当前优化边界。
